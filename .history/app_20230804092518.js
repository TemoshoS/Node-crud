...
//getting data in the add-job page
app.get('/add-job',async(req,res)=>{
  ...
  try {
    ...
    res.render('addjob', { title: 'Add job' ,responseArray})
...
app.get('/',async(req,res)=>{
  ...
  try {
    ...
    res.render('index', { students, title: 'Home' ,responseArray})
    //res.status(200).send(responseArray)
...
app.post('/add-job',async(req, res)=>{
  ...
  try {
    ...
    const response = await db.collection('jobs').add(jobJson).then(()=>{
      res.redirect('addjob')
    })
